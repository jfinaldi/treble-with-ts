...
export const validMessages: MessageHandler[] = [
  GetSongHandler,
  GetAllSongsHandler,
  GetRandomSongsHandler,
  SearchSongsHandler,
...
app.get("/drums", (req: any, res: any) => {
  ...
  const filePath: string =
    path.join(__dirname, "../") + "sounds/drums/" + beat_file;
  fs.readFile(
    filePath,
    function (err: any, file: Buffer | ArrayBuffer | string | any) {
      var base64File: string | undefined = Buffer.from(file, "binary").toString(
        "base64"
      );
...
app.get("/three", (req: any, res: any) => {
  ...
  const returnData: any = {};
  const filePath: string =
    path.join(__dirname, "../") + "sounds/cat/" + beat_file;
...
app.get("/flute", (req: any, res: any) => {
  ...
  const returnData: any = {};
  const filePath: string =
    path.join(__dirname, "../") + "sounds/flute/" + flute_sound;
  fs.readFile(filePath, function (err: any, file: any) {
    var base64File = Buffer.from(file, "binary").toString("base64");
    returnData.fileContent = base64File;
    res.json(returnData);
...
app.get("/xylophone", (req: any, res: any) => {
  ...
  const returnData: any = {};
  const filePath: string =
    path.join(__dirname, "../") + "sounds/xylophone/" + beat_sound;
  fs.readFile(filePath, function (err: any, file: any) {
    var base64File = Buffer.from(file, "binary").toString("base64");
    returnData.fileContent = base64File;
    res.json(returnData);
